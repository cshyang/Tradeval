...
export function ExperimentBuilder({ onClose }: { onClose: () => void }) {
  ...
  const [mode, setMode] = useState<"hindsight" | "forward">("hindsight");
  ...
  return <div className="builder-backdrop" onMouseDown={onClose}><div className="builder" role="dialog" aria-modal="true" aria-label="Build agent experiment" onMouseDown={(e) => e.stopPropagation()}>
    ...
    {step === "configure" ? <><div className="builder-fields"><label>Capital<input value="$100,000" readOnly /></label><label>Cadence<input value="Monthly" readOnly /></label><label>Mode<select value={mode} onChange={(e) => setMode(e.target.value as "hindsight" | "forward")}><option value="hindsight">Hindsight scenario</option><option value="forward">Forward paper</option></select></label><label>Max position<input type="number" value={position} onChange={(e) => setPosition(Number(e.target.value))} /></label><label>Max turnover<input type="number" value={turnover} onChange={(e) => setTurnover(Number(e.target.value))} /></label></div><div className="builder-candidates"><div className="builder-kicker">CANDIDATE PREVIEW · APPROXIMATELY 12</div>{symbols.map((symbol) => <span key={symbol}>{symbol}</span>)}</div></> : null}
    {step === "run" ? <><ExperimentProgress events={events} status={status} classification={mode === "forward" ? "FORWARD PAPER" : "HINDSIGHT SCENARIO"} /><div className="builder-notes"><AgentDecisionPanel /><InterventionPanel /></div></> : null}
    ...
    <footer>{step !== "describe" && step !== "run" ? <button className="secondary" onClick={() => setStep(step === "review" ? "describe" : "review")}>Back</button> : <span />}
      {step === "describe" ? <button onClick={() => void interpret()}>Interpret philosophy</button> : step === "review" ? <button onClick={() => setStep("configure")}>Configure experiment</button> : step === "configure" ? <button onClick={() => void run()}>{mode === "forward" ? "Schedule forward paper" : "Start hindsight scenario"}</button> : <button onClick={onClose}>View equity replay</button>}</footer>
  </div></div>;
}
