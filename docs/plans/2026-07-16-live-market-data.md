...
> **Merge reconciliation note:** This plan records the feature-branch design.
> Current `main` has a stricter resume contract: manifest, philosophy, event,
> provenance, and materialized-artifact mismatches fail before writes. The
> feature branch's automatic transition-journal rematerialization was therefore
> not retained because it would heal materialized mismatches during resume.
> Resume instead validates the event sequence, reconstructs the ledger, and
> compares every materialized row before restoring state. The synthetic
> lifecycle also keeps its established scheduled date as the execution session,
> using the preceding completed session for its decision; real-price weekly
> replay uses the next actual SPY-calendar session.
