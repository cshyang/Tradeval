...
def test_market_replay_uses_real_price_shape_fake_cash_and_provenance(
    ...
    assert query.end == date(2024, 4, 30) + timedelta(days=7)
    ...
    equity_lines = (run_dir / "equity.csv").read_text().strip().splitlines()
    assert equity_lines[0] == "date,equity,spy_equity,equal_weight_equity"
    equity_rows = equity_lines[1:]
    ...
    assert "Return vs SPY" in report
    assert (workspace / "comparison.md").exists()

    export_dir = tmp_path / "public" / "runs"
    _export_workspace(workspace, export_dir)
    view_model = json.loads((export_dir / "data.json").read_text())
    assert view_model["data_provenance"]["kind"] == "real_market"
    assert view_model["data_provenance"]["provider"] == "yfinance"
    assert view_model["experiments"][0]["data_provenance"]["label"] == (
        "HINDSIGHT · ADJUSTED MARKET DATA"
    )
